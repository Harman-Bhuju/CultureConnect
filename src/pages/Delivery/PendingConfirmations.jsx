  ...
  const fetchDeliveries = async () => {
    try {
      ...
      const response = await fetch(
        `${API.GET_SHIPPED_ORDERS}?status=delivered_pending`,
        {
          credentials: "include",
        },
      );
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 flex gap-4 items-start">
        ...
        <div>
          <h4 className="font-bold text-blue-900 text-sm">
            Instruction for Delivery Boy
          </h4>
          <p className="text-blue-700 text-xs mt-1 leading-relaxed">
            Please ask the buyer to check their email (
            <strong>
              {deliveries.length > 0
                ? "Look for 'Delivery Confirmation'"
                : "customer's email"}
            </strong>
            ) and click the "Confirm Receipt" button. Once they confirm, the
            order will be marked as completed and will disappear from this list.
          </p>
        </div>
      </div>
      ...
      {deliveries.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-slate-300">
          ...
          <p className="text-slate-400">
            There are no orders awaiting confirmation right now.
          </p>
        </div>
      ) : (
                ...
                <div className="pt-4 border-t border-slate-100 space-y-4">
                  {delivery.has_report ? (
                    <button
                      ...
                      <AlertTriangle className="w-3 h-3" /> Resolve Discrepancy
                      First
                    </button>
                  ) : (
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (data.status === "success") {
                            ...
                            toast.error(
                              data.message || "Failed to resend link",
                              { id: toastId },
                            );
                          }
                        } catch (err) {
                  ...
                  <div className="flex items-center gap-2 text-amber-600">
                    ...
                    <span className="text-xs font-bold italic">
                      Waiting for customer to click the email link...
                    </span>
                  </div>
                </div>
              </div>
