...
const Navbar = () => {
  ...
  const getSearchContext = () => {
    ...
    if (path.includes("/marketplace") || path.includes("/products") || path.includes("/sellerprofile")) {
      let category = "";
    ...
    if (path.includes("/learnculture") || path.includes("/courses") || path.includes("/teacherprofile")) {
      let category = "";
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          <div className="flex items-center gap-1 flex-shrink-0">
            <div className="flex items-center lg:hidden md:gap-2 sm:gap-1">
              ...
              <img src={CultureConnectLogo} alt="Logo" className="w-4 h-4 md:w-6 md:h-6 object-contain" />
              <span className="text-sm md:text-[18px] font-bold text-gray-800">CultureConnect</span>
            </div>
            {!isSearchOpen && isCollapsed && (
              <div className="hidden lg:flex items-center gap-2">
                <img src={CultureConnectLogo} alt="Logo" className="w-8 h-8 object-contain" />
                <span className="text-xl font-bold text-gray-800">CultureConnect</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-1 md:gap-3 flex-1 justify-end">
            ...
            {isMobileSearch && !isSearchOpen && (
              <button onClick={handleSearchToggle} className="p-2 hover:bg-gray-100 rounded-full"><Search className="w-5 h-5" /></button>
            )}

            {!isSearchOpen && (
              <>
                <button className="relative p-2 hover:bg-gray-100 rounded-full" onClick={() => navigate("/cart")}>
                  <ShoppingCart className="w-5 h-5" />
                </button>
                <button onClick={() => navigate("/notifications")} className="relative p-2 hover:bg-gray-100 rounded-full">
                  <Bell className="w-5 h-5" /><span className="absolute top-2 right-0 bg-red-500 w-2 h-2 rounded-full"></span>
                </button>
                <button className="flex-shrink-0">
                  <img src={user?.avatar || default_logo} alt="User" className="w-7 h-7 md:w-10 md:h-10 rounded-full border-2 border-gray-200" />
                </button>
              </>
            )}
          </div>
      ...
      {isSearchOpen && isMobileSearch && (
        <div className="fixed inset-0 bg-white z-[100] flex flex-col p-4">
          <div className="flex items-center gap-3 mb-4">
            <button onClick={handleSearchToggle} className="p-3 hover:bg-gray-100 rounded-full"><X size={28} /></button>
