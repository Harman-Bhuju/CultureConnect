 ...
 */
const SearchBar = ({ variant = "navbar", contextType = "all", initialCategory = "" }) => {
    ...
    useEffect(() => {
        const fetchSuggestions = async () => {
            ...
            try {
                ...
                const response = await fetch(`${API.GET_SEARCH_SUGGESTIONS}?${params.toString()}`);
    ...
    const handleSearchSubmit = (suggestion = null) => {
        const queryText = (suggestion && typeof suggestion === "object") ? suggestion.text : (typeof suggestion === "string" ? suggestion : searchQuery);
        if (!queryText.trim()) return;

        let targetPath = location.pathname;
        const category = (suggestion && typeof suggestion === "object") ? suggestion.category : initialCategory;
    ...
    const themeColor = variant === "hero-marketplace" ? "orange" : (variant === "hero-learn" ? "teal" : "blue");

    return (
        <div className={`relative w-full ${isHero ? "max-w-2xl mx-auto" : "max-w-xl"} ${isOpen ? "z-[100]" : "z-10"}`} ref={containerRef}>
            {/* Search Input Container */}
            <div className={`relative group ${isHero ? "z-20" : ""}`}>
                {isHero && (
                    <div className={`absolute -inset-1 bg-gradient-to-r from-${themeColor}-500/20 to-amber-500/20 rounded-[2rem] blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200`}></div>
                )}
                ...
                <div className="relative">
                    <input
                        ...
                        placeholder={isHero ? "Search for heritage, crafts, masters..." : `Search ${initialCategory ? "in " + initialCategory : contextType}...`}
                        className={`w-full ${isHero ? "pl-14 pr-24 py-5 rounded-[1.5rem] text-lg bg-white" : "pl-4 pr-20 py-2 rounded-full text-sm md:text-base bg-gray-50 md:bg-white"} border border-gray-100 md:border-gray-300 focus:outline-none focus:ring-2 focus:ring-${themeColor}-500/20 focus:border-${themeColor}-500 transition-all hover:shadow-md`}
                    />

                    <Search className={`absolute ${isHero ? "left-5" : "right-3"} top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-${themeColor}-500 transition-colors ${isHero ? "w-6 h-6" : "w-5 h-5 cursor-pointer"}`}
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                        ...
                        <button onClick={startVoiceSearch} className={`p-2 rounded-full hover:bg-gray-100 ${isListening ? "text-red-500 animate-pulse" : "text-gray-400"}`}>
            ...
            {isOpen && suggestions.length > 0 && (
                <div className={`absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 shadow-xl z-[100] overflow-hidden ${isHero ? "rounded-2xl" : "rounded-lg"}`}>
                    {!searchQuery && (
                    ...
                    <div className="max-h-[300px] overflow-y-auto">
                        {suggestions.map((s, i) => (
                            ...
                            >
                                <Search className={`text-gray-300 ${isHero ? "w-5 h-5" : "w-4 h-4"}`} />
                                <div className="flex flex-col">
                                    <span className={`font-medium text-gray-900 ${isHero ? "text-base" : "text-sm"}`}>{s.text}</span>
                                    {s.category && (
                                        <span className="text-[10px] text-gray-400 uppercase font-semibold">{s.category}</span>
                                    )}
                                </div>
                            </button>
                        ))}
